...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Hint do campo link (exibe somente quando vazio)
  const hintLink = document.getElementById('hintLink') || document.querySelector('.form-hint');

  function updateHintVisibility() {
    if (!hintLink) return;
    if (inputLink.value.trim() === '') {
      // usar display padrão (deixa o CSS controlar)
  ...
  inputLink.addEventListener('input', updateHintVisibility);
  ...
  function criarGrupoDoModal() {
    ...
    // Limpa campos e fecha modal
    inputNome.value = inputDescricao.value = inputCriadoPor.value = inputLink.value = '';
    // garante que o hint volte a aparecer ao limpar o campo
    updateHintVisibility();
  ...
  function openViewModalFromLi(li) {
    ...
    // esconder o hint quando estiver em modo view (campo readonly)
  ...
  function resetModalToCreateMode() {
    try {
      ...
      // Restaurar visibilidade do hint conforme conteúdo do campo
      updateHintVisibility();
